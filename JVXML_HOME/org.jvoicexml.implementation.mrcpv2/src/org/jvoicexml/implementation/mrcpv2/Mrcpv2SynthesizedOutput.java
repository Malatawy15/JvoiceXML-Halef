/*
 * File:    $HeadURL: https://svn.code.sf.net/p/jvoicexml/code/trunk/org.jvoicexml.implementation.mrcpv2/src/org/jvoicexml/implementation/mrcpv2/Mrcpv2SynthesizedOutput.java $
...
public final class Mrcpv2SynthesizedOutput
	...
	try {
	       ...
	       if (ssml.getSpeak().getChildNodes() != null)
	       {
		int childNodeCount = ssml.getSpeak().getChildNodes().getLength();
		if (childNodeCount > 0 )
		{
			XmlNode xmlNode = (XmlNode) ssml.getSpeak().getChildNodes().item(0);
			url = xmlNode.getAttribute("src");
			if (url != null)
			{
				isUrl = true;
			}
		...
		speakText = url;
	       }else
	       {
	       	speakText = ssml.getSpeak().getTextContent();
              	LOGGER.info("Using TTS!: " + speakText);
	       }
	    }
            //play the text
	    //ispeechClient.queuePrompt(true, "http://it-tjr7.dhbw-stuttgart.de/1.wav");
            //speechClient.queuePrompt(false, speakText);
            speechClient.queuePrompt(isUrl, speakText);
